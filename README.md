...
| SSL | `certbot_certs` | сертификаты Let's Encrypt |
...
backups/certs-20260623-120000.tar.gz
backups/db-latest.sql.gz          → симлинк на последний дамп
backups/uploads-latest.tar.gz
backups/certs-latest.tar.gz
```

Если SSL ещё не настраивали (нет volume `certbot_certs`), режим `all` пропустит certs с предупреждением.
...
# восстановить БД, медиа и сертификаты
...
Если восстановили `certs` — **`init-letsencrypt.sh` не нужен**, nginx подхватит сертификат сам. Запускайте его только для **первого** выпуска SSL на новом домене.

Без бэкапа сертификатов:

```bash
./infra/init-letsencrypt.sh   # новый запрос к Let's Encrypt
```
...
| `CERTBOT_CERTS_VOLUME` | — | имя volume вручную, если автоопределение не сработало |
