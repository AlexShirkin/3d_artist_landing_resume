...
| БД | **PostgreSQL 16** |
| Логи (prod) | **Dozzle** — веб-UI логов контейнеров |
| Аналитика | **Яндекс.Метрика** (опционально, через `YM_COUNTER_ID`) |
...
infra/expand-ssl-domains.sh — добавить LOGS_DOMAIN в сертификат
infra/setup-dozzle-auth.sh — создать пользователя Dozzle
...
### 4. Логирование для разработчика (Dozzle)

**Dozzle** — лёгкий веб-интерфейс для просмотра логов всех Docker-контейнеров в реальном времени (`web`, `nginx`, `gateway`, `postgres` и т.д.).

| | Яндекс.Метрика | Dozzle |
|--|----------------|--------|
| Для кого | посетители сайта | разработчик / админ |
| Что показывает | визиты, источники | stdout/stderr контейнеров |
| Где | счётчик на лендинге | `https://logs.ваш-домен` |

Логин и пароль задаются в `.env` (`LOGS_USER`, `LOGS_PASSWORD`), **не коммитятся в git**.  
Аутентификация — встроенная в Dozzle (файл `users.yml` в volume `dozzle_data`).
...
LOGS_DOMAIN=logs.portfolio.example.com
LOGS_USER=logs
LOGS_PASSWORD=надёжный-пароль-без-символа-$
```

2. DNS: A-запись `LOGS_DOMAIN` → IP сервера.
...
4. Создайте пользователя Dozzle (обязательно — без `users.yml` контейнер не стартует):
...
5. Откройте `https://logs.ваш-домен` → форма Dozzle → логин/пароль из `.env`.

#### Уже работающий сервер (добавить Dozzle)
...
# 2. Расширить SSL (если logs. ещё не в сертификате)
chmod +x infra/expand-ssl-domains.sh
./infra/expand-ssl-domains.sh

# 3. Поднять Dozzle и nginx
...
Измените `LOGS_PASSWORD` в `.env` и снова:
...
| Симптом | Решение |
|---------|---------|
| Dozzle в статусе `Restarting` | Нет `users.yml` → `./infra/setup-dozzle-auth.sh` |
| Форма логина снова и снова | Неверный пароль или пересоздайте пользователя скриптом выше |
| Пароль со спецсимволами `$`, `#` | Может ломаться в `.env` — временно используйте простой пароль |
| Не открывается `https://logs.` | Проверьте DNS, `./infra/expand-ssl-domains.sh`, `docker compose ps nginx dozzle` |

**Логи самого Dozzle** (если UI не открывается):

```bash
docker compose -f docker-compose.yml -f docker-compose.prod.yml logs --tail=50 dozzle
```

**Без Dozzle** — логи в SSH:

```bash
docker compose -f docker-compose.yml -f docker-compose.prod.yml logs -f web
docker compose -f docker-compose.yml -f docker-compose.prod.yml logs -f gateway portfolio-service
```
...
Интернет :443
  ...
  nginx ──► web:3000      (DOMAIN)
         ...
         └──► dozzle:8080 (LOGS_DOMAIN, вход Dozzle)
```
...
**Скачать на Mac** (подставьте ключ, пользователя и хост):

```bash
mkdir -p ~/cloth-backups

rsync -avz --progress -e "ssh -i ~/.ssh/develop" \
...
Можно заходить как `root` с личным ключом — тогда `root@SERVER` вместо `deploy@SERVER`.
...
A-записи `DOMAIN`, `ADMIN_DOMAIN` и (если используете Dozzle) `LOGS_DOMAIN` должны указывать на **новый IP**.
...
| Добавили `LOGS_DOMAIN`, его нет в certs | `./infra/expand-ssl-domains.sh` |
| Нужны логи в браузере | `./infra/setup-dozzle-auth.sh` после поднятия `dozzle` |
...
| 6 | DNS на новый IP (`DOMAIN`, `ADMIN_DOMAIN`, `LOGS_DOMAIN`) |
| 7 | `up -d postgres portfolio-service certbot` |
| 8 | `./infra/restore.sh all ...` |
| 9 | `up -d --build` весь стек |
| 10 | SSL: без `init-letsencrypt.sh`, если certs восстановлены; иначе `expand-ssl-domains.sh` для `logs.` |
| 11 | `./infra/setup-dozzle-auth.sh` — если нужен Dozzle |
